...
test("global search renders correctly at product viewports", async ({ page }, testInfo) => {
  ...
  for (const viewport of [
    ...
    await page.goto("/workspaces/company/tasks?task=task-e2e");
    await expect(page.getByRole("dialog", { name: "Prepare candidate review" })).toBeVisible();
    await page.getByLabel("Task title").focus();
    await page.keyboard.press("Control+k");
    const palette = page.getByRole("dialog", { name: "Search ProveIt" });
...
test("workspace tasks, meetings, and activity routes work", async ({ page }) => {
  ...
  await expect(page.getByLabel("Change status for Prepare candidate review")).toHaveValue("in_progress");
  await page.reload();
  await page.getByRole("button", { name: /List/ }).click();
  await expect(page.getByLabel("Change status for Prepare candidate review")).toHaveValue("in_progress");
  ...
  const meetingDialog = page.getByRole("dialog", { name: "Meeting details" });
  await expect(meetingDialog).toBeVisible();
  await expect(meetingDialog.getByText("Mentioned User", { exact: true })).toBeVisible();
  await page.getByLabel("Meeting status").selectOption("completed");
  const saveMeeting = page.getByRole("button", { name: "Save" });
  await saveMeeting.click();
  await expect(saveMeeting).toHaveText("Save");
  await meetingDialog.getByRole("link", { name: "Open full page", exact: true }).click();
...
test("a task opens as a route-backed side peek and expands to its direct page", async ({ page }) => {
  ...
  await page.goBack();
  await page.getByRole("link", { name: "Open task full page" }).click();
  await expect(page).toHaveURL(/\/workspaces\/company\/tasks\/task-e2e$/);
