...
export async function syncMappedBusinessTask(taskId: string, fields: unknown, projects: { business: string; technology: string }) {
  ...
  if (!snapshot.exists || task?.workspaceId !== "business" || !task.integration?.kaneo?.taskId) return { state: "partial" as SyncState, message: "External sync is not configured for this task." };
  if (task.integration.kaneo.projectId !== getKaneoProjectIdForWorkspace("business", projects)) throw new Error("Business routing is unavailable.");
...
export async function deleteMappedBusinessTask(taskId: string, projects: { business: string; technology: string }) {
  const ref = adminDb.collection("tasks").doc(taskId); const snapshot = await ref.get(); const task = snapshot.data();
  if (!snapshot.exists || task?.workspaceId !== "business" || !task.integration?.kaneo?.taskId) return { state: "partial" as SyncState };
  if (task.integration.kaneo.projectId !== getKaneoProjectIdForWorkspace("business", projects)) throw new Error("Business routing is unavailable.");
