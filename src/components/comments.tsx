...
export function Comments({ workspaceId, entityType, entityId }: { workspaceId: string; entityType: EntityType; entityId: string }) {
  ...
  return <section className="mt-8 border-t border-[var(--border)] pt-5"><div className="flex items-baseline justify-between"><h2 className="proveit-section-title">Comments</h2><span className="text-xs text-[var(--subtle)]">{comments.length || "No"} {comments.length === 1 ? "comment" : "comments"}</span></div><div className="mt-4 space-y-5">{roots.map((comment) => <Thread key={comment.id} comment={comment} replies={comments.filter((reply) => reply.parentCommentId === comment.id)} onReply={setReplyingTo} />)}{!error && roots.length === 0 && <p className="py-3 text-sm text-[var(--muted)]">Start the conversation for this record.</p>}</div><div className="mt-5 border-t border-[var(--border)] pt-4">{replyingTo && <p className="mb-2 text-xs text-[var(--muted)]">Replying to <strong className="font-medium text-[var(--foreground)]">{replyingTo.authorName}</strong> <button onClick={() => setReplyingTo(null)} className="ml-1 underline hover:text-[var(--foreground)]">Cancel</button></p>}<textarea aria-label="Comment" value={body} onChange={(event) => setBody(event.target.value)} placeholder={replyingTo ? "Write a reply…" : "Add a comment…"} className="min-h-24 w-full resize-y rounded-md border border-transparent bg-[var(--sidebar)] px-3 py-2.5 text-sm leading-6 outline-none transition placeholder:text-[var(--subtle)] focus:border-[var(--focus)] focus:bg-white" /><div className="mt-2 flex items-center justify-between gap-3"><select aria-label="Mention teammate" value={mention} onChange={(event) => setMention(event.target.value)} className="rounded-md bg-[var(--sidebar)] px-2 py-1.5 text-sm"><option value="">Mention someone…</option>{users.filter((user) => user.active && user.uid !== firebaseUser?.uid).map((user) => <option key={user.uid} value={user.uid}>@{user.name}</option>)}</select><button data-testid="comment-submit" onClick={postComment} disabled={!body.trim() || posting} className="proveit-primary-button disabled:cursor-not-allowed disabled:opacity-50">{posting ? "Posting…" : replyingTo ? "Reply" : "Comment"}</button></div>{error && <p role="alert" className="mt-2 text-sm text-[var(--danger)]">{error}</p>}</div></section>;
}
...
function CommentItem({ comment, onReply }: { comment: Comment; onReply: (comment: Comment) => void }) {
  return <div className="group flex gap-3"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[var(--active)] text-[11px] font-medium text-[var(--muted)]">{comment.authorName.slice(0, 1).toUpperCase()}</span><div className="min-w-0 flex-1"><div className="flex items-baseline gap-2"><p className="text-sm font-medium">{comment.authorName}</p><time className="text-xs text-[var(--subtle)]">{comment.createdAt?.toLocaleString() || "Just now"}</time></div><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-[var(--foreground)]">{comment.body}</p><button onClick={() => onReply(comment)} className="mt-1.5 text-xs text-[var(--muted)] opacity-0 transition hover:text-[var(--foreground)] focus:opacity-100 group-hover:opacity-100">Reply</button></div></div>;
}
