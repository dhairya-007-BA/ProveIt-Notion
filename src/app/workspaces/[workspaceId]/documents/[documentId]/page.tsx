...
export default function DocumentEditorPage() {
  ...
  if (authLoading || loading) return <main className="grid min-h-screen place-items-center bg-[var(--background)] text-sm text-[var(--muted)]">Loading document…</main>;
  if (!firebaseUser || !profile) return null;

  return <RecordDetailShell backHref={`/workspaces/${workspaceId}/documents`} backLabel="Documents" actions={<span className="px-2 py-1 text-xs text-[var(--subtle)]">{saveStatus === "saving" ? "Saving…" : saveStatus === "saved" ? "Saved" : "Unsaved changes"}</span>}>
    ...
    <RecordContentSection title="Content"><textarea aria-label="Document content" value={content} onChange={(event) => { setContent(event.target.value); setError(""); }} placeholder="Start writing…" className="min-h-[55vh] w-full resize-y rounded bg-transparent px-1 py-2 text-base leading-8 text-[var(--text)] outline-none placeholder:text-[var(--subtle)] hover:bg-[var(--hover)] focus:bg-[var(--input)] focus-visible:ring-2 focus-visible:ring-[var(--focus)]/35" /></RecordContentSection>
