...
export default function RowPage() {
  ...
  if (loading) return <main className="grid min-h-screen place-items-center bg-[#fbfbfa] text-sm text-[#787774]">Loading row…</main>;
  if (error || !row || !database) return <main className="grid min-h-screen place-items-center bg-[#fbfbfa] text-sm text-red-700">{error || "Row not found"}</main>;
...
function PropertyEditor({ property, value, onChange }: { property: Property; value: string | number | boolean | null | undefined; onChange: (value: string | number | boolean | null) => void }) {
  const controlClass = "w-full rounded bg-transparent px-2 py-1 text-sm text-[#37352f] outline-none hover:bg-[#f1f1ef] focus:bg-[#f1f1ef] focus-visible:ring-2 focus-visible:ring-[#2383e2]/35";
  if (property.type === "checkbox") return <input aria-label={property.name} type="checkbox" checked={Boolean(value)} onChange={(event) => onChange(event.target.checked)} className="h-4 w-4 accent-[#2383e2]" />;
