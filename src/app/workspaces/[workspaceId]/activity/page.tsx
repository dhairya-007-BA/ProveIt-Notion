...
export default function ActivityPage() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(query(collection(db, "activity"), where("workspaceId", "==", workspaceId)), (snapshot) => setEvents(snapshot.docs.map((item) => ({ id: item.id, description: item.data().description || "Updated workspace", entityType: item.data().entityType || "workspace", createdAt: item.data().createdAt?.toDate() })).sort((left, right) => (right.createdAt?.getTime() || 0) - (left.createdAt?.getTime() || 0))), (listenerError) => {
  ...
  return <main className="flex min-h-screen bg-[var(--background)]"><Sidebar /><section className="min-w-0 flex-1 px-5 py-7 sm:px-8 md:px-10"><div className="mx-auto max-w-3xl"><Link href={`/workspaces/${workspaceId}`} className="text-sm text-[var(--muted)] transition hover:text-[var(--foreground)]">← Back to workspace</Link><header className="mt-7"><p className="proveit-label">Workspace</p><h1 className="proveit-page-title mt-1">Recent activity</h1><p className="mt-2 text-sm text-[var(--muted)]">A chronological record of work in this workspace.</p></header><div className="mt-8">{error && <p role="alert" className="border-b border-[var(--border)] py-4 text-sm text-[var(--danger)]">{error}</p>}{events.length > 0 && <p className="proveit-label border-b border-[var(--border)] pb-2">Recent</p>}{events.map((event) => <article key={event.id} className="flex gap-3 border-b border-black/[0.07] py-4"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[var(--subtle)]" /><div><p className="text-sm">{event.description}</p><p className="mt-1 text-xs text-[var(--muted)]">{event.createdAt?.toLocaleString() || "Just now"}</p></div></article>)}{!error && events.length === 0 && <p className="py-10 text-sm text-[var(--muted)]">No recorded activity yet.</p>}</div></div></section></main>;
}
