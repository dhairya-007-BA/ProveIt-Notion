...
export default function ActivityPage() { const { workspaceId } = useParams<{ workspaceId: string }>(); const router = useRouter(); const { firebaseUser, profile, loading } = useAuth(); const [events, setEvents] = useState<Event[]>([]); const [error, setError] = useState("");
  ...
  useEffect(() => { if (!firebaseUser || !profile) return; return onSnapshot(query(collection(db, "activity"), where("workspaceId", "==", workspaceId)), (snapshot) => setEvents(snapshot.docs.map((item) => ({ id: item.id, description: item.data().description || "Updated workspace", entityType: item.data().entityType || "workspace", createdAt: item.data().createdAt?.toDate() })).sort((a, b) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0))), (listenerError) => { console.error("Failed to load activity:", listenerError); setError("Activity could not be loaded."); }); }, [firebaseUser, profile, workspaceId]);
