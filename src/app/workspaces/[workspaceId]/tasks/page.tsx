...
import { syncBusinessTaskUpdate } from "@/lib/kaneo-business-task-update-sync";
...
export default function TasksPage() {
  ...
  async function moveTask(taskId: string, next: EditableTaskStatus) {
    ...
    try {
      ...
      const result = firebaseUser ? await syncBusinessTaskUpdate(firebaseUser, workspaceId, taskId, ["status"]) : null;
  ...
  const activeFilters = [status !== "all" && ["Status", status, () => setStatus("all")] as const, priority !== "all" && ["Priority", priority, () => setPriority("all")] as const, assignee !== "all" && ["Assignee", assignee === "unassigned" ? "Unassigned" : employeeOptions.find((item) => item.uid === assignee)?.name || "Assigned", () => setAssignee("all")] as const, due !== "all" && ["Due", due === "none" ? "No due date" : due, () => setDue("all")] as const].filter(Boolean) as [string, string, () => void][];
...
function MenuButton({ label, open, setOpen, children }: { label: string; open: boolean; setOpen: (value: boolean) => void; children: React.ReactNode }) {
  ...
  return <div ref={ref} className="relative"><button type="button" aria-expanded={open} onClick={() => setOpen(!open)} className="proveit-secondary-button whitespace-nowrap">{label}</button>{open && <div role="menu" aria-label={label} className="absolute right-0 z-30 mt-2 w-56 rounded-xl border border-[var(--border)] bg-[var(--surface-elevated)] p-2 shadow-[var(--shadow-md)]">{children}</div>}</div>;
}
...
function Card({ task, workspaceId, preferences, customFields = [], onOpen }: { task: EditableTask; workspaceId: string; preferences: Preferences; customFields?: WorkspaceCustomField[]; onOpen: () => void }) { const shown = customFields.filter((field) => preferences.customFieldIds.includes(field.id)).slice(0, 3); return <button type="button" onClick={onOpen} className="group block w-full rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 text-left shadow-[var(--shadow-sm)] transition hover:-translate-y-px hover:border-[var(--secondary)] hover:shadow-[var(--shadow-md)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--secondary)]"><p className="line-clamp-2 text-sm font-medium leading-5">{task.title}</p>{preferences.description && task.description && <p className="mt-1 line-clamp-2 text-xs leading-5 text-[var(--muted)]">{task.description}</p>}<div className="mt-3 flex min-w-0 items-center gap-2 text-xs text-[var(--muted)]">{preferences.priority && <span className={`proveit-priority proveit-priority-${task.priority}`}>{task.priority}</span>}{preferences.due && task.dueDate && <span className="shrink-0">{task.dueDate.toLocaleDateString()}</span>}{preferences.assignee && <AssigneeName uid={task.assigneeId} />}<span className="ml-auto"><Sync task={task} workspaceId={workspaceId} /></span></div>{shown.length > 0 && <div className="mt-3 space-y-1 border-t border-[var(--border)] pt-2 text-xs text-[var(--muted)]">{shown.map((field) => <div key={field.id} className="flex gap-2"><span className="truncate">{field.name}</span><CustomValue field={field} value={task.customFields?.[field.id]} /></div>)}</div>}</button>; }
function Board({ tasks, preferences, customFields, dragged, setDragged, onMove, onOpen, onCreate }: { tasks: EditableTask[]; preferences: Preferences; customFields: WorkspaceCustomField[]; dragged: string | null; setDragged: (id: string | null) => void; onMove: (id: string, status: EditableTaskStatus) => void; onOpen: (task: EditableTask) => void; onCreate: (status: EditableTaskStatus) => void }) { return <div className="mt-5 grid grid-cols-1 gap-3 pb-2 sm:grid-cols-2 xl:grid-cols-4">{columns.map((column) => { const items = tasks.filter((task) => task.status === column.status); return <section key={column.status} onDragOver={(event) => event.preventDefault()} onDrop={() => { if (dragged) onMove(dragged, column.status); setDragged(null); }} className="min-h-56 rounded-xl border border-[var(--border)] bg-[var(--sidebar)] p-3"><header className="flex min-h-8 items-center gap-2 px-1"><h2 className="proveit-heading text-xs font-semibold uppercase tracking-wide">{column.label}</h2><span className="grid h-5 min-w-5 place-items-center rounded-full bg-[var(--surface-elevated)] px-1.5 text-[11px] text-[var(--muted)]">{items.length}</span><button type="button" onClick={() => onCreate(column.status)} aria-label={`Add task to ${column.label}`} className="ml-auto grid h-8 w-8 place-items-center rounded-md text-[var(--secondary)] transition hover:bg-[var(--hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--secondary)]">+</button></header><div className="mt-3 space-y-2">{items.map((task) => <div key={task.id} draggable onDragStart={() => setDragged(task.id)} className={`cursor-grab active:cursor-grabbing ${dragged === task.id ? "opacity-50" : ""}`}><Card task={task} workspaceId={task.workspaceId} preferences={preferences} customFields={customFields} onOpen={() => onOpen(task)} /></div>)}{items.length === 0 && <button type="button" onClick={() => onCreate(column.status)} className="w-full rounded-lg py-7 text-sm text-[var(--muted)] transition hover:bg-[var(--hover)] hover:text-[var(--secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--secondary)]">No tasks<br /><span className="text-xs">+ Add task</span></button>}</div></section>; })}</div>; }
function List({ tasks, preferences, customFields, onOpen, onMove }: { tasks: EditableTask[]; preferences: Preferences; customFields: WorkspaceCustomField[]; onOpen: (task: EditableTask) => void; onMove: (id: string, status: EditableTaskStatus) => void }) { const shown = customFields.filter((field) => preferences.customFieldIds.includes(field.id)); return <div className="mt-5 overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--surface)]"><table className="w-full min-w-[760px] text-sm"><thead className="border-b border-[var(--border)] text-left text-xs text-[var(--muted)]"><tr><th className="px-4 py-3">Task</th><th className="px-4 py-3">Status</th>{preferences.priority && <th className="px-4 py-3">Priority</th>}{preferences.assignee && <th className="px-4 py-3">Assignee</th>}{preferences.due && <th className="px-4 py-3">Due date</th>}{shown.map((field) => <th key={field.id} className="px-4 py-3">{field.name}</th>)}</tr></thead><tbody>{tasks.map((task) => <tr key={task.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--hover)]"><td><button type="button" onClick={() => onOpen(task)} className="w-full px-4 py-3 text-left font-medium">{task.title}</button></td><td className="px-4"><select value={task.status} aria-label={`Status for ${task.title}`} onChange={(event) => onMove(task.id, event.target.value as EditableTaskStatus)} className="bg-transparent text-sm"><option value="todo">To do</option><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="done">Done</option></select></td>{preferences.priority && <td className="px-4"><span className={`proveit-priority proveit-priority-${task.priority}`}>{task.priority}</span></td>}{preferences.assignee && <td className="px-4"><AssigneeName uid={task.assigneeId} /></td>}{preferences.due && <td className="px-4 text-[var(--muted)]">{task.dueDate?.toLocaleDateString() || "—"}</td>}{shown.map((field) => <td key={field.id} className="max-w-48 px-4"><CustomValue field={field} value={task.customFields?.[field.id]} /></td>)}</tr>)}</tbody></table></div>; }
function Drawer({ task, workspaceId, onClose }: { task: EditableTask; workspaceId: string; onClose: () => void }) { return <aside aria-label="Task detail panel" className="fixed inset-y-0 right-0 z-50 flex w-full max-w-[620px] flex-col border-l border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-md)]"><header className="flex shrink-0 items-start justify-between gap-3 border-b border-[var(--border)] px-4 py-4 sm:px-5"><div className="min-w-0 flex-1"><p className="proveit-label">{workspaceId} · task</p><p className="mt-1 truncate text-sm font-medium">{task.title}</p></div><div className="flex shrink-0 items-center gap-3"><Link href={`/workspaces/${workspaceId}/tasks/${task.id}`} className="proveit-secondary-button hidden sm:inline-flex">Open full page</Link><button type="button" onClick={onClose} className="proveit-secondary-button" aria-label="Close task detail">Close <span aria-hidden="true">×</span></button></div></header><div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-5"><section aria-label="Task properties" className="mb-5 rounded-lg border border-[var(--border)] bg-[var(--surface-muted)]"><div className="grid grid-cols-[6rem_minmax(0,1fr)] gap-3 px-4 py-3 text-sm"><span className="text-[var(--muted)]">Assignee</span><AssigneeName uid={task.assigneeId} /></div></section><EditTaskForm key={task.id} task={task} onCancel={onClose} onSaved={() => {}} onDeleted={onClose} /><section className="mt-8 border-t border-[var(--border)] pt-5"><h2 className="proveit-heading text-base font-semibold">Activity &amp; comments</h2><Comments workspaceId={workspaceId} entityType="task" entityId={task.id} /></section></div></aside>; }
