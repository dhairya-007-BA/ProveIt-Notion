...
export default function TasksPage() {
  ...
  return <main className="flex min-h-screen bg-[var(--background)]"><Sidebar /><section className="min-w-0 flex-1 px-5 py-7 sm:px-8 md:px-10"><div className={`mx-auto ${selected ? "max-w-none" : "max-w-[1400px]"}`}><Link href={`/workspaces/${workspaceId}`} className="text-sm text-[var(--muted)] transition hover:text-[var(--foreground)]">← Back to workspace</Link><header className="mt-7 flex items-end justify-between gap-4"><div><p className="proveit-label">Tasks</p><h1 className="proveit-page-title mt-1">Tasks</h1></div><button onClick={() => setCreating(true)} className="proveit-primary-button">New task</button></header><div className="mt-7 flex border-b border-[var(--border)]"><ViewButton active={view === "table"} onClick={() => setView("table")}>▦ Table</ViewButton><ViewButton active={view === "board"} onClick={() => setView("board")}>▤ Board</ViewButton></div>{error && <p role="alert" className="mt-3 text-sm text-[var(--danger)]">{error}</p>}<div className={selected ? "grid min-w-0 grid-cols-[minmax(0,1fr)_minmax(380px,42%)]" : ""}><div className="min-w-0">{view === "table" ? <TaskTable tasks={tasks} onOpen={openTask} onStatus={moveTask} /> : <TaskBoard tasks={tasks} draggedId={draggedId} setDraggedId={setDraggedId} onDrop={moveTask} onOpen={openTask} />}</div>{selected && <aside aria-label="Task detail pane" className="min-h-[calc(100vh-10rem)] border-l border-[var(--border)] bg-white px-5 py-4"><div className="sticky top-0 z-10 flex justify-between bg-white pb-3"><button aria-label="Close task pane" onClick={closeTask} className="proveit-secondary-button">× Close</button><Link aria-label="Expand task" href={`/workspaces/${workspaceId}/tasks/${selected.id}`} className="proveit-secondary-button">↗ Expand</Link></div><div className="max-h-[calc(100vh-13rem)] overflow-y-auto pr-1"><EditTaskForm task={selected} onCancel={closeTask} onSaved={() => {}} onDeleted={closeTask} /></div></aside>}</div></div></section>{creating && <div role="dialog" aria-modal="true" aria-label="New task" className="fixed inset-0 z-50 grid place-items-start overflow-y-auto bg-black/20 px-4 py-8 sm:place-items-center"><div className="w-full max-w-2xl"><NewTaskForm workspaceId={workspaceId} currentUser={firebaseUser} onCancel={() => setCreating(false)} onCreated={() => setCreating(false)} /></div></div>}</main>;
}
...
function TaskTable({ tasks, onOpen, onStatus }: { tasks: EditableTask[]; onOpen: (task: EditableTask) => void; onStatus: (id: string, status: EditableTaskStatus) => void }) {
  return <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="border-b border-[var(--border)] text-left text-xs font-medium text-[var(--muted)]"><tr><th className="px-3 py-2.5">Task</th><th className="px-3 py-2.5">Status</th><th className="px-3 py-2.5">Priority</th><th className="px-3 py-2.5">Due date</th></tr></thead><tbody>{tasks.map((task) => <tr key={task.id} className="border-b border-black/[0.07] transition hover:bg-[var(--hover)]"><td><button className="w-full px-3 py-3 text-left font-medium" onClick={() => onOpen(task)}>{task.title}</button></td><td className="px-3 py-2"><select aria-label={`Status for ${task.title}`} value={task.status} onChange={(event) => onStatus(task.id, event.target.value as EditableTaskStatus)} className="rounded-md bg-transparent px-2 py-1 capitalize hover:bg-white"><option value="todo">Not started</option><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="done">Done</option></select></td><td className="px-3 py-2"><span className={`proveit-priority proveit-priority-${task.priority}`}>{task.priority}</span></td><td className="px-3 py-2 text-[var(--muted)]">{task.dueDate?.toLocaleDateString() || "—"}</td></tr>)}</tbody></table>{tasks.length === 0 && <p className="px-3 py-10 text-sm text-[var(--muted)]">No tasks yet. Create the first task for this workspace.</p>}</div>;
}
