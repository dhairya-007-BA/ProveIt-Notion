...
import { createKaneoBusinessTask, KaneoTaskCreateError } from "@/lib/kaneo-task-create";
...
import { KaneoTaskMappingError, linkKaneoTaskToProveItTask } from "@/lib/kaneo-task-mapping";
...
async function parseBusinessTaskBody(request: Request): Promise<BusinessTaskBody | null> {
  let body: unknown;
  try {
  ...
  if ((body.proveItTaskId !== undefined && (typeof body.proveItTaskId !== "string" || !body.proveItTaskId.trim())) || typeof body.title !== "string" || !body.title.trim()) return null;
  ...
  return {
    ...(body.proveItTaskId === undefined ? {} : { proveItTaskId: body.proveItTaskId.trim() }),
    title: body.title.trim(),
    ...(body.description === undefined ? {} : { description: body.description.trim() }),
    ...(body.priority === undefined ? {} : { priority: body.priority as BusinessTaskBody["priority"] }),
  };
}
...
function productionErrorResponse(error: unknown) {
  ...
  if (error instanceof KaneoTaskMappingError) {
    return response({ success: false, message: "Kaneo task was created, but its ProveIt link could not be confirmed." }, error.status);
  }
  if (error instanceof KaneoRoutingError) {
    return response({ success: false, message: "Business workspace routing is unavailable." }, 422);
  }
...
export async function POST(request: Request) {
  const body = await parseBusinessTaskBody(request);
  if (!body) {
    return response({ success: false, message: "A valid Business task is required." }, 422);
  }

  try {
    const user = await requireKaneoWorkspaceAccess(request, BUSINESS_WORKSPACE_ID);
    const task = await createKaneoBusinessTask(body);
    if (body.proveItTaskId) {
      await linkKaneoTaskToProveItTask(body.proveItTaskId, user.uid, {
        taskId: task.id,
